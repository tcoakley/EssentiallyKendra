...
class cTableAssistant {
	...
	private $arrImageDimensions = array();
	...
	function DoInsert() {
		...
		for ($looper = 0; $looper < count($this->arrFieldsSet); $looper ++) {
			...
			if (is_array($_FILES[$FieldName]['tmp_name'])) {
				$TempName = $_FILES[$FieldName]['tmp_name'][$looper];
			} else {
			...
			if ($FieldName != $this->PrimaryKey) {
				...
				switch ($FieldType) {
					...
					case "file":
						if(strlen($TempName) > 0 || !is_null($this->MasterImageName)) {
							if (strlen($_FILES[$FieldName]['tmp_name'][$looper]) > 0 ) {
								if (is_array($_FILES[$FieldName]['name'])) {
									$FieldValue = $_FILES[$FieldName]['name'][$looper];
								} else {
	...
	function DoModify() {
		...
		for ($looper = 0; $looper < count($this->arrFieldsSet); $looper ++) {
			...
			$FieldValue = $this->$FieldName;
			$FieldType = $this->arrDBFieldTypes[$looper];
			$DeclaredFieldType = $this->arrFieldTypes[$looper];
			$FieldLength = $this->arrFieldLengths[$looper];
			...
			if ($FieldName != $this->PrimaryKey) {
				switch ($FieldType) {
					...
					case "int":  
						if ($this->arrFieldTypes[$looper] == "bool") {
							if($FieldValue != 1) {
								$FieldValue = 0;
							}
						}
	...
	function SaveFiles() {
		if ($this->HasFiles) {
			for ($looper = 0; $looper < count($this->arrFields); $looper ++) {
				...
				if($FieldType == "file") {
					if((!is_null($TempPath) && strlen($TempPath) > 3) || !is_null($this->MasterImageName)) {
						...
						if ($ImageDimensions != null) {
							...
							if ($cWidth < $width || $cHeight < $height) {
								if ($this->AutoCrop) {
									...
									debug("width", $width);
									debug("height", $height);
									debug("CutWidth", $CutWidth);
									debug("CutHeight", $CutHeight, false);
									*/
			...
			//debug("ImageDimensions", $ImageDimensions);
			//debug("DestFile", $DestFile,true);
		}
	}
			...
			for ($looper = 0; $looper < count($this->arrFields); $looper ++) {
				...
				if($FieldType == "file") {
					if(strlen($FieldValue) > 4) {
						$DestFile = GetWebRoot() . str_replace("\\","/",$FileSavePath) . $FieldName . $this->TableID . "." . GetFileExtension($FieldValue);
						unlink($DestFile);
	...
	function GetImageSize($FieldName, &$width, &$height, &$type = null, &$attr = null) {
		//debug("this->GetFileName($FieldName, false)", $this->GetFileName($FieldName, true));
